...
export default function SpeakerModal({ speakerId, onClose, zIndex = 100 }: SpeakerModalProps) {
  ...
  const links         = parseLinks((speaker?.links as unknown as string) ?? null);
  const liveSessions  = speaker?.sessions?.filter((s: { live: any; }) => s.live)  ?? [];
  const otherSessions = speaker?.sessions?.filter((s: { live: any; }) => !s.live) ?? [];
