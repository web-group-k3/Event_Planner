...
interface SessionModalProps {
  ...
  onClose: () => void;
  /** z-index level — increase when nested inside another modal */
  zIndex?: number;
  /** Called when user clicks a speaker inside this modal */
  onSpeakerClick?: (id: string) => void;
}

function QuestionRow({ q }: { q: Question }) {
  return (
    <div className="flex gap-4 p-4 glass rounded-2xl border border-white/08 hover:border-white/15 transition-all duration-200">
...
export default function SessionModal({
  ...
  // Portal requires document — only render client-side
    ...
    >
      ...
      <div
        className={`
          ...
          md:rounded-[40px] border border-white/10
        ...
        {!loading && session && (
          <div className="flex-1 overflow-y-auto">
            ...
            <div className="relative px-8 pt-10 pb-8 border-b border-white/08">
              <div className="absolute top-0 left-0 w-96 h-64 bg-[#a3ff12]/04 blur-[120px] pointer-events-none" />
              ...
              <div className="mt-6 flex flex-wrap gap-5">
                ...
                {session.guestNumber != null && (
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 rounded-xl bg-white/05 flex items-center justify-center">
                ...
                {(session.speakers?.length ?? 0) > 0 ? (
                  <div className="flex flex-col gap-3">
                    {session.speakers!.map((spk) => {
                      const links = parseLinks(spk.links);
                      const initials = spk.fullName.split(" ").map(n => n[0]).join("").toUpperCase().slice(0, 2);
                      return (
                        <button
                          key={spk.id}
                          onClick={() => onSpeakerClick?.(spk.id)}
                          disabled={!onSpeakerClick}
                          className="flex items-center gap-4 p-4 glass rounded-2xl border border-white/10
                                     hover:border-[#a3ff12]/40 transition-all duration-200 text-left group
                                     disabled:cursor-default"
                        >
                          <div className="w-12 h-12 rounded-xl overflow-hidden bg-linear-to-br from-[#a3ff12]/20 to-[#38bdf8]/20 border border-white/10 flex items-center justify-center shrink-0">
                            {spk.photoUrl ? (
